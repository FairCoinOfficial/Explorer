import { getCachedBlock, getCachedBlockHash, getCachedTransaction } from '@/lib/cached-rpc';
...
import { Hash, Clock, Database, Link, ArrowLeft, ArrowRight } from 'lucide-react';

type Params = { params: { hashOrHeight: string } };

async function resolveBlockHash(hashOrHeight: string): Promise<string> {
  if (/^\d+$/.test(hashOrHeight)) {
    const height = parseInt(hashOrHeight, 10);
    return await getCachedBlockHash(height);
  }
  return hashOrHeight;
}

export default async function BlockPage({ params }: Params) {
  const hash = await resolveBlockHash(params.hashOrHeight);
  const block = await getCachedBlock(hash);
